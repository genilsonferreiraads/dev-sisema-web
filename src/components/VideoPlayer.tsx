...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ onEnded, isPlaying, setIsPlaying, pendingVideoId }): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const handleTTSVolumeChange = (event: CustomEvent) => {
      if (isPlayerReady && !isInternalStateChange) {
        const { shouldReduceVolume } = event.detail;
    ...
    window.addEventListener('ttsVolumeChange', handleTTSVolumeChange as EventListener);

    return () => {
      window.removeEventListener('audioPlay', handleAudioPlay);
      window.removeEventListener('audioStop', handleAudioStop);
      window.removeEventListener('ttsVolumeChange', handleTTSVolumeChange as EventListener);
    };
